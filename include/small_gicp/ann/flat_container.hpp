...
public:
  ...
  /// @brief Which point survives when two land close together in a cell.
  enum class InsertPolicy {
    FirstCome,        ///< As vendored: the point already there wins, the new one is dropped.
    CenterPreference  ///< Whichever point better represents the cell centre wins.
  };
  ...
  struct Setting {
    ...
    InsertPolicy policy = InsertPolicy::FirstCome;
  };
  ...
  void add(const Setting& setting, const Eigen::Vector4d& transformed_pt, const PointCloud& points, size_t i, const Eigen::Isometry3d& T,
           const Eigen::Vector4d& cell_center = Eigen::Vector4d::Zero()) {
    // Index of the point this one is too close to, if any.
    size_t conflict = this->points.size();
    for (size_t n = 0; n < this->points.size(); n++) {
      if ((this->points[n] - transformed_pt).squaredNorm() < setting.min_sq_dist_in_cell) {
        conflict = n;
        break;
      }
    }
    const bool has_conflict = conflict < this->points.size();

    if (setting.policy == InsertPolicy::CenterPreference && has_conflict) {
      // Order-independent: keep whichever point sits nearer the cell centre.
      // First-come instead lets whichever scan saw the surface first hold the
      // cell, and a surface is usually seen from far away first, so the noisier
      // observation wins and every later close-range one is thrown out.
      const double incoming = (transformed_pt - cell_center).head<3>().squaredNorm();
      const double resident = (this->points[conflict] - cell_center).head<3>().squaredNorm();
      if (incoming >= resident) {
        return;
      }
      this->points[conflict] = transformed_pt;
      if constexpr (HasNormals) {
        this->normals[conflict] = T.matrix() * traits::normal(points, i);
      }
      if constexpr (HasCovs) {
        this->covs[conflict] = T.matrix() * traits::cov(points, i) * T.matrix().transpose();
      }
      return;
    }

    if (has_conflict || this->points.size() >= setting.max_num_points_in_cell) {
      return;
    }
